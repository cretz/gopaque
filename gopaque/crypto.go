...
func (c *CryptoStandard) AuthEncrypt(priv kyber.Scalar, plain []byte) ([]byte, error) {
	...
	mac := hmac.New(c.Hash, unsafeMarshal(macKey))
	mac.Write(encBytes)
...
func DeriveKeyHKDF(c Crypto, priv kyber.Scalar, info []byte) kyber.Scalar {
	hkdfR := hkdf.New(c.Hash, unsafeMarshal(priv), nil, info)
...
// DeriveKeyArgonDefault is the default DeriveKeyArgon parameters.
...
func (d *DeriveKeyArgon) DeriveKey(c Crypto, priv kyber.Scalar, info []byte) kyber.Scalar {
	...
	h := c.Hash()
	h.Write(info)
	argonHash := argon2.IDKey(unsafeMarshal(priv), h.Sum(nil), d.Time, d.Memory, d.Threads, uint32(c.ScalarLen()))
